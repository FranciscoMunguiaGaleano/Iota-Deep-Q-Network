...
from game import Pacman_v1_1 as env
from agents import *
...
class DDDQL:
    def __init__(self):
        print("Dueling Double Deep Q-Learning Pacman environment training")
        pygame.init()
        pygame.display.set_caption("Pacman")
        self.experiment_name="dddql"
        ...
        self.agente=SimpleDuelingAgent(n_actions=self.n_actions,input_shape=s.flatten().shape,buffer_size=self.max_queue_length,batch_size=self.mini_batch_size)
    ...
    def episode_training(self,mode):
        ...
        if mode=="random":
            ...
            name=self.experiment_name+"_random_episodes"
            epsilon=1.0
        else:
...
if __name__ == "__main__":
    print("Training Pacman environment with DDDQL")
    env=DDDQL()
    env.epoch_training()
