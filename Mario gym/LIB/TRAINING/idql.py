...
class IDQL:
    def __init__(self ,lambda_1=1.0,lambda_2=1.0,lambda_3=0.0,lambda_="l1"):
        print("Iota Deep Q-Learning mario environment training")
        pygame.init()
        pygame.display.set_caption("MEXITENDO")
        self.lambda_1=lambda_1
        self.lambda_2=lambda_2
        self.lambda_3=lambda_3
        self.experiment_name="idql"+lambda_
        self.experiment_name_="idql"
        ...
        while episode<self.max_episodes:
            ...
            while not d:
                ...
                if normal:
                    ...
                    action,nn=self.agente.epsilon_policy(self.agente.epsilon,s,iota)                
                s_,r,d,_s=self.environment.step(self.actions[action])
                self.environment.render()
                iota_=self.environment.iota()[1:]
                rwd+=r
                if nn:
...
def save_data(data):
    with open(r''+ENV_PATH+'/tmp/idql.yaml', 'w') as outfile:
        data={'avr_r':data}
        yaml.dump(data, outfile, default_flow_style=False)
...
if __name__ == "__main__":
    ...
    env.environment.visualize=False
    env.epoch_training()
