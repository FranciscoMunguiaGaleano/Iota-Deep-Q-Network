...
from game import Mario_v1_1 as envi
from agents import *
...
class IDDQL:
    def __init__(self ,lambda_1=1.0,lambda_2=1.0,lambda_3=0.0,lambda_="l1"):
        print("Iota Double Deep Q-Learning mario environment training")
        pygame.init()
        pygame.display.set_caption("MEXITENDO")
        self.lambda_1=lambda_1
        self.lambda_2=lambda_2
        self.lambda_3=lambda_3
        self.experiment_name="iddql"+lambda_
        self.experiment_name_="iddql"
        ...
        self.agente=IotaQAgent(n_actions=self.n_actions,input_shape=s.flatten().shape,buffer_size=self.max_queue_length,batch_size=self.mini_batch_size,lambda_1=self.lambda_1,lambda_2=self.lambda_2,lambda_3=self.lambda_3)
    ...
    def epoch_training(self):
        ...
        maxq=[]
        eoe_maxq=[]
        #for stepation in range(0,480000):
        while epoch<self.max_epochs:
            ...
            while not d:
                ...
                s_,r,d,info=self.environment.step(self.actions[action])
                self.environment.render()
                iota_=self.environment.iota()[1:]
                rwd+=r
                if nn:
                    rwd_nn+=r
                ...
                if step_num%6==0 and step_num>1:
                      q_max,loss_t=self.agente.train_double()
                      weight_updates+=1
        ...
        while episode<self.max_episodes:
            ...
            while not d:
                ...
                if normal:
                    ...
                    action,nn=self.agente.epsilon_policy(self.agente.epsilon,s,iota)                
                s_,r,d,_=self.environment.step(self.actions[action])
                self.environment.render()
                iota_=self.environment.iota()[1:]
                rwd+=r
                if nn:
...
def save_data(data):
    with open(r''+ENV_PATH+'/tmp/iddql.yaml', 'w') as outfile:
        data={'avr_r':data}
        yaml.dump(data, outfile, default_flow_style=False)
...
if __name__ == "__main__":
    ...
    env.epoch_training()
    save_data(env.environment.avg_reward)
